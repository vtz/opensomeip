...
platform::ByteBuffer EventGroupEntry::serialize() const {
    ...
    const uint16_t packed = static_cast<uint16_t>(((reserved_12bit_ & 0x0FFFU) << 4U) |
                                                  (static_cast<uint16_t>(counter_) & 0x0FU));
    data[12] = static_cast<uint8_t>((packed >> 8U) & 0xFFU);
    data[13] = static_cast<uint8_t>(packed & 0xFFU);
...
bool EventGroupEntry::deserialize(const platform::ByteBuffer& data, size_t& offset) {
    ...
    ttl_ = (static_cast<uint32_t>(data[offset + 5]) << 16U) | (static_cast<uint32_t>(data[offset + 6]) << 8U) |
           static_cast<uint32_t>(data[offset + 7]);
    const uint16_t packed = static_cast<uint16_t>((static_cast<uint32_t>(data[offset + 8]) << 8U) |
                                                  static_cast<uint32_t>(data[offset + 9]));
    reserved_12bit_ = static_cast<uint16_t>((packed >> 4U) & 0x0FFFU);
    counter_ = static_cast<uint8_t>(packed & 0x0FU);
