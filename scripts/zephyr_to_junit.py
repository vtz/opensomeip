...
def parse_zephyr_log(log_text: str, suite_name: str) -> ET.Element:
    ...
    for line in log_text.splitlines():
        ...
        if pass_match:
            tests += 1
            ET.SubElement(testsuite, "testcase", name=pass_match.group(1).strip(), classname=suite_name)
        elif fail_match:
            tests += 1
            failures += 1
            tc = ET.SubElement(testsuite, "testcase", name=fail_match.group(1).strip(), classname=suite_name)
            ET.SubElement(tc, "failure", message="Test failed")

    if tests == 0:
        results_match = re.search(r"=== Results:\s*(\d+)\s*passed,\s*(\d+)\s*failed\s*===", log_text)
        if results_match:
            ...
            for i in range(failed):
                failures += 1
                tc = ET.SubElement(testsuite, "testcase", name=f"failed_test_{i+1}", classname=suite_name)
                ET.SubElement(tc, "failure", message="Test failed")
