...
################################################################################
#
# Build and run FreeRTOS SOME/IP tests on Renode (STM32F407 Cortex-M4F).
#
# Usage:
#   ./scripts/run_freertos_renode_test.sh [OPTIONS]
#
# Options:
...
BUILD_DIR="$PROJECT_DIR/build/freertos-cortexm4-renode"
...
ELF_PATH="$BUILD_DIR/renode/test_freertos_renode.elf"

echo "=== FreeRTOS Renode Test (STM32F407 Cortex-M4F) ==="
echo "  Timeout: ${TIMEOUT}s"

# --- Build ---
if [ "$SKIP_BUILD" = false ]; then
    echo "  Building with preset freertos-cortexm4-renode..."
    cmake --preset freertos-cortexm4-renode -S "$PROJECT_DIR" 2>&1
...
# --- Run Renode ---
LOGFILE=$(mktemp /tmp/renode_freertos_XXXXXX.log)
...
if [ -n "$JUNIT_OUTPUT" ] && [ -f "$LOGFILE" ]; then
    ...
    python3 "$SCRIPT_DIR/zephyr_to_junit.py" "$LOGFILE" "freertos_renode" "$JUNIT_OUTPUT" || echo "  WARNING: JUnit XML generation failed"
fi

echo "=== FreeRTOS Renode test complete ==="
