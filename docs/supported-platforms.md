...
```mermaid
graph TB
    subgraph ECU1["ECU 1 — Head Unit"]
        direction TB
        NAV["Navigation App"]
        C1["SOME/IP Client"]
        S1["SOME/IP Stack<br/>RPC + Transport + SD"]
    end

    subgraph ECU2["ECU 2 — Instrument Cluster"]
        direction TB
        DISP["Display App"]
        C2["SOME/IP Client"]
        S2["SOME/IP Stack<br/>RPC + Transport + SD"]
    end

    subgraph ECU3["ECU 3 — Central Gateway"]
        direction TB
        GW["Gateway Service"]
        SRV1["SOME/IP Server"]
        S3["SOME/IP Stack<br/>RPC + Transport + SD"]
    end

    subgraph ECU4["ECU 4 — Powertrain"]
        direction TB
        ENG["Engine Control"]
        SRV2["SOME/IP Server"]
        S4["SOME/IP Stack<br/>RPC + Transport + SD"]
    end

    subgraph NET["Automotive Ethernet"]
        direction LR
        SW["Ethernet Switch<br/>VLAN + QoS"]
    end
    ...
    style ECU1 fill:#e3f2fd,color:#333,stroke:#1565c0
    style ECU2 fill:#e8f5e9,color:#333,stroke:#2e7d32
    style ECU3 fill:#fff3e0,color:#333,stroke:#e65100
    style ECU4 fill:#fce4ec,color:#333,stroke:#ad1457
    style NET fill:#f5f5f5,color:#333,stroke:#616161
```
