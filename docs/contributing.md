...
Fedora 42 ships GCC 15, Clang 20, and CMake 3.31 — all well above the minimum requirements.

```bash
sudo dnf install -y gcc gcc-c++ clang cmake ninja-build git
```

### Docker Dev Environments

Pre-configured Docker environments are available for reproducible builds:

```bash
# Ubuntu-based dev environment (default)
docker-compose -f docker-compose.dev.yml run --rm dev bash

# Fedora 42 dev environment (GCC 15 + Clang 20)
docker-compose -f docker-compose.dev.yml run --rm fedora-dev bash
```
