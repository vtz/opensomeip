...
| **Buffer overflow in caller-provided buffer (outbound)** — caller passes undersized `buf` / `buf_len` to get-payload, get-data, or serialize | `opensomeip_message_get_payload`, `opensomeip_serializer_get_data`, `opensomeip_message_serialize` | Out-of-bounds write: memory corruption, possible code execution. | Unit tests `TC_CAPI_MSG_PAYLOAD_OVERFLOW_001`, `TC_CAPI_SER_OVERFLOW_001` verify `OPENSOMEIP_RESULT_BUFFER_OVERFLOW` is returned without writing past the buffer. ASan in CI. | All copy-out functions check `*out_len < actual_size` **before** any `memcpy` and return `OPENSOMEIP_RESULT_BUFFER_OVERFLOW` with `*out_len` set to required size. No data is written when the buffer is too small. | **S4** (if unmitigated); **S1** (with bounds check) |
| **Buffer overflow in handler bridge (inbound)** — RPC method handler returns SUCCESS with `*output_len` exceeding the bridge buffer capacity (`OPENSOMEIP_RPC_METHOD_MAX_RESPONSE`) | `opensomeip_rpc_server_register_method` handler bridge | Stack buffer overflow in the bridge lambda: memory corruption, possible code execution. | Unit test `TC_CAPI_RPC_HANDLER_OVERFLOW_001` verifies that a handler returning an oversized `*output_len` produces `INTERNAL_ERROR` without writing past the buffer. ASan in CI. | The bridge clamps `out_len` against `sizeof(out_buf)` after the handler returns; if `out_len > sizeof(out_buf)` the bridge returns `RpcResult::INTERNAL_ERROR` without performing `memcpy`. The handler also runs inside an FFI exception firewall (`try/catch(...)`). `OPENSOMEIP_RPC_METHOD_MAX_RESPONSE` is documented in the public header. | **S4** (if unmitigated); **S1** (with clamp + firewall) |
...
| Buffer overflow (outbound) | **Negligible** | Bounds check before every `memcpy`. ASan CI. |
| Buffer overflow (inbound handler) | **Negligible** | Handler bridge clamps returned length against `OPENSOMEIP_RPC_METHOD_MAX_RESPONSE`; exception firewall on handler invocation. ASan CI. |
