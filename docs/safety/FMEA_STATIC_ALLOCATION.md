...
| Ref count overflow | **Low** | Debug-mode assert (`assert(prev < UINT16_MAX)`) catches saturation during development. In production, bounded message lifetime and protocol exchange patterns limit concurrent references well below 65534. Residual risk exists if an application creates >65534 concurrent `MessagePtr` copies — impractical in normal SOME/IP workloads but documented. |
